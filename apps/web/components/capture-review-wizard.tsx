...
type SpeciesEntryMode = "catalog" | "custom";
...
export function CaptureReviewWizard({ review, confirmExistingTreeAction, createTreeFromCaptureAction }: CaptureReviewWizardProps) {
  ...
  const [speciesEntryMode, setSpeciesEntryMode] = useState<SpeciesEntryMode>("catalog");
  const [customSpeciesLabel, setCustomSpeciesLabel] = useState("");
  const [customSpeciesScientificName, setCustomSpeciesScientificName] = useState("");
  const [copySpeciesProfileFromId, setCopySpeciesProfileFromId] = useState("");
  ...
  const trimmedCustomSpeciesLabel = customSpeciesLabel.trim();
  const trimmedCustomSpeciesScientificName = customSpeciesScientificName.trim();
  const selectedSpeciesLabel = speciesEntryMode === "custom"
    ? (trimmedCustomSpeciesLabel.length > 0 ? trimmedCustomSpeciesLabel : null)
    : selectedSpecies?.label ?? null;
  const selectedSpeciesDisplayLabel = speciesEntryMode === "custom"
    ? (trimmedCustomSpeciesLabel.length > 0
      ? formatCatalogDisplayLabel(trimmedCustomSpeciesLabel, trimmedCustomSpeciesScientificName || null)
      : null)
    : (selectedSpecies ? formatCatalogDisplayLabel(selectedSpecies.label, selectedSpecies.subtitle ?? null) : null);
    ...
    <div className="capture-wizard-shell">
      ...
      {review.captureIntent === "new-tree" && currentStep === "species-manual" ? (
        <section className="capture-card capture-wizard-step">
          ...
          <p>Choose the species from the list, or add it manually if it is not there.</p>
          {speciesEntryMode === "catalog" ? (
            <>
              <SpeciesSearchPicker
            ...
            <div className="inline-form">
              <label className="field-block">
                ...
                <input
                  onChange={(event) => {
                    setCustomSpeciesLabel(event.target.value);
                  }}
                  placeholder="Example: Mountain hemlock"
                  value={customSpeciesLabel}
                />
              </label>
              <label className="field-block">
                ...
                <input
                  onChange={(event) => {
                    setCustomSpeciesScientificName(event.target.value);
                  }}
                  placeholder="Optional, for example Tsuga mertensiana"
                  value={customSpeciesScientificName}
                />
              </label>
              <label className="field-block">
                <span>Start with guidance from</span>
                <select
                  onChange={(event) => {
                    setCopySpeciesProfileFromId(event.target.value);
                  }}
                  value={copySpeciesProfileFromId}
                >
                  <option value="">Leave care and notes blank</option>
                  {review.speciesCatalog.map((entry) => (
                    <option key={entry.id} value={entry.id}>
                      {formatCatalogDisplayLabel(entry.label, entry.subtitle ?? null)}
                    </option>
                  ))}
                </select>
              </label>
              <p className="helper-text">Copy care, characteristics, bonsai notes, and the seasonal guide from a close species, or leave them blank for now.</p>
              <button
                className="button button-ghost"
                onClick={() => {
                  setSpeciesEntryMode("catalog");
          ...
          <div className="capture-step-actions">
            ...
            <button
              className="button button-solid"
              disabled={speciesEntryMode === "catalog" ? !selectedSpeciesId : trimmedCustomSpeciesLabel.length === 0}
      ...
      {review.captureIntent === "new-tree" && currentStep === "name" ? (
        <section className="capture-card capture-wizard-step">
          ...
          <form action={createTreeFromCaptureAction} className="inline-form">
            <input name="submissionId" type="hidden" value={review.id} />
            <input name="speciesEntryMode" type="hidden" value={speciesEntryMode} />
            <input name="speciesId" type="hidden" value={speciesEntryMode === "catalog" ? selectedSpeciesId : ""} />
            <input name="customSpeciesLabel" type="hidden" value={customSpeciesLabel} />
            <input name="customSpeciesScientificName" type="hidden" value={customSpeciesScientificName} />
            <input name="copySpeciesProfileFromId" type="hidden" value={copySpeciesProfileFromId} />
